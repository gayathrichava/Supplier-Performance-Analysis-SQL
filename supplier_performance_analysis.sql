...
FROM supplier_kpi_raw 
GROUP BY quarter, INITCAP(LOWER(TRIM(supplier)))
HAVING COUNT(*) >= 5
ORDER BY supplier_clean, quarter;
...
-- 3. Highest problem-rate supplier
SELECT
    INITCAP(LOWER(TRIM(supplier))) AS supplier_clean,
    COUNT(*) AS total_kpis,
    COUNT(*) FILTER (WHERE rating <> 'Good') AS problem_kpis,
...
-- 4. Highest problem-rate business areas
SELECT
    business_area,
    COUNT(*) AS total_kpis,
    COUNT(*) FILTER (WHERE rating <> 'Good') AS problem_kpis,
